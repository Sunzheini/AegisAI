...
class User(BaseModel):
    ...
    id: Optional[int] = Field(
        default=None, ge=1, description="Auto-generated positive integer ID"
    )
    name: str = Field(min_length=1, max_length=100, description="User's full name")
    age: int = Field(ge=0, le=120, description="User's age between 0 and 120")
    city: str = Field(min_length=1, max_length=100, description="City name")
    email: Optional[str] = Field(
        default=None, description="Valid email address if provided"
    )
    ...
    def as_form(
        ...
        email: Optional[str] = Form(
            None, description="Valid email address if provided"
        ),
...
class UserUpdate(BaseModel):
    ...
    password: Optional[str] = Field(
        default=None, min_length=8, description="User's new password (optional)"
    )
