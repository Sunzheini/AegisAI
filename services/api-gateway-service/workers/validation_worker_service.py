    ...
    async def _validate_image_file(self, file_path: str) -> list:
        ...
        try:
            ...
            if (
                ...
                errors.append(
                    f"Image file size suggests dimensions may exceed "
                    f"maximum allowed {self.max_image_dimension}x{self.max_image_dimension}"
                )
    ...
    async def _validate_video_file(self, file_path: str) -> list:
        ...
        try:
            ...
            if estimated_duration > self.max_video_duration:
                errors.append(
                    f"Estimated video duration ({estimated_duration:.1f}s) "
                    f"may exceed maximum allowed {self.max_video_duration}s"
                )
