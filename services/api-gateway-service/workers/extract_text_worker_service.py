...
class ExtractTextService(INeedRedisManagerInterface):
    ...
    async def _extract_text_from_pdf(file_path: str) -> dict:
        ...
        try:
                ...
                for page_num, page in enumerate(pdf.pages):
                    try:
                        ...
                        if page_text.strip():
                            ...
                            result["extracted_text"] += f"--- Page {page_num + 1} ---\n{page_text}\n\n"
            ...
            result["extraction_errors"].append("pdfplumber not installed. Install with: pip install pdfplumber")
        except Exception as e:
    ...
    @staticmethod
    async def _save_extracted_text_to_file(job_id: str, extracted_text: str, character_count: int) -> str:
    ...
    async def _analyze_text_content(extracted_text: str) -> dict:
        ...
        try:
            ...
            technical_terms = ["microcontroller", "datasheet", "voltage", "circuit", "processor"]
    ...
    async def _process_extract_text_worker(self, state: WorkflowGraphState) -> WorkflowGraphState:
        ...
        try:
            ...
            if not os.path.exists(file_path):
                ...
                errors.append(f"Text extraction only supported for PDF files. Got: {state['content_type']}")
            else:
                ...
                if extraction_result["character_count"] > 0:
                    ...
                    text_analysis = await self._analyze_text_content(extraction_result["extracted_text"])
        ...
        if errors:
            ...
            state["metadata"]["errors"] = errors    # Preserve existing metadata but add errors

            if extraction_result:   # Still include any partial extraction results if available
            ...
            state["metadata"]["extract_text"] = "passed"  # Don't overwrite m-data, add success flag
