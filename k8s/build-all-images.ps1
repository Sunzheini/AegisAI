...
param(
    ...
    [string]$Registry = "sunzheini1407",  # Updated to correct username
...
foreach ($service in $services) {
    ...
    # Fix: Don't add '-service' suffix if service name already ends with '-service'
    ...
    try {
        ...
        Write-Host "  [OK] Built successfully in $($buildTime.TotalSeconds.ToString('F2'))s" -ForegroundColor Green
...
if ($buildErrors.Count -gt 0) {
    ...
    foreach ($error in $buildErrors) {
        Write-Host "  [ERROR] $($error.Service): $($error.Error)" -ForegroundColor Red
    }
    Write-Host ""
...
foreach ($service in $services) {
    $imageName = "aegisai-$($service.ImageName)"
    $fullImageName = "${Registry}/${imageName}:${Tag}"
