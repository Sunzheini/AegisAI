...
class BaseWorkerClient(INeedRedisManagerInterface):
    ...
    async def process_file_by_the_worker(self, state: WorkflowGraphState, timeout: int = 30) -> WorkflowGraphState:
        """
        ...
        try:
            ...
            print(f"[{self.worker_name}] Published {self.task_name} task for job_id: {job_id}")
            ...
            print(f"[{self.worker_name}Client] Error during {self.task_name} for job {job_id}: {e}")
            raise
                    ...
                    if message.get("type") == "message":
                        try:
                            ...
                            if data.get("job_id") == job_id:
                                print(f"[{self.worker_name}Client] Received {self.task_name} result for job_id: {job_id}")
                                return data["result"]
                        except (json.JSONDecodeError, KeyError) as e:
                            print(f"[{self.worker_name}Client] Invalid message format: {e}")
                            continue
        except asyncio.TimeoutError:
            raise TimeoutError(f"{self.task_name.capitalize()} service timeout for job {job_id}")
        finally:
