def test_orchestrator_logging_setup(caplog, orchestrator_client):
    ...
    with caplog.at_level("INFO", logger="workflow-orchestrator"):
        ...
        assert any("Orchestrator test log" in record.message for record in caplog.records)
...
def test_orchestrator_health_endpoint_logging(caplog, orchestrator_client):
    ...
    with caplog.at_level("INFO", logger="workflow-orchestrator"):
        ...
        request_logs = [
            ...
            if "request received" in r.getMessage().lower() and r.name == "workflow-orchestrator"
        ]
        ...
        response_logs = [
            r for r in caplog.records
            if "response sent" in r.getMessage().lower() and r.name == "workflow-orchestrator"
        ]
        assert len(response_logs) > 0, "No response logs found for workflow-orchestrator"
